...
for a,b in prev_pairs:
    if a in student_ids and b in student_ids:
        pair_same = model.NewBoolVar(f"prevsame_{a}_{b}")
        zlist = []
        for c in range(NUM_CLASSROOMS):
            z = model.NewBoolVar(f"z_{a}_{b}_c{c}")
            zlist.append(z)
            model.Add(z <= assigned[a][c])
            model.Add(z <= assigned[b][c])
            model.Add(assigned[a][c] + assigned[b][c] - z <= 1)
        model.Add(sum(zlist) == pair_same)
        pair_same_vars.append(pair_same)
...
obj_terms = []
if pair_same_vars:
    obj_terms.append((weight_prev, sum(pair_same_vars)))
obj_terms.append((weight_range, max_avg - min_avg))


expr = None
for w, term in obj_terms:
    if expr is None:
        expr = term * w
    else:
        expr = expr + term * w
...
print("Solving (soft prev penalty)...")
status = solver.Solve(model)

if status in (cp_model.OPTIMAL, cp_model.FEASIBLE):
    print("Solution found:", solver.StatusName(status))
    print("Objective value (weighted):", solver.ObjectiveValue())
